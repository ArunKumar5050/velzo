...
export const EditProduct = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
          ...
          // Handle old string category format gracefully
          categories: Array.isArray(product.category) ? product.category : (product.category ? [product.category] : []),
  ...
  const removeImage = () => {
    setFormData((prev) => ({
      ...prev,
      imageFile: null,
      imageUrl: '', // Clear the image to allow uploading a new one
    }))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.categories && formData.categories.length > 0) {
        productData.category = formData.categories
      } else {
