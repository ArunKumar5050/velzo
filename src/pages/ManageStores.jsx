...
import { getMedicalStores, updateMedicalStore, deleteMedicalStore } from '../services/medicalStoreService';
import { createMedicalStorePartner } from '../services/partnerService';
...
export default function ManageStores() {
  ...
  const fetchStores = async () => {
    try {
      setLoading(true);
      const data = await getMedicalStores();
      setStores(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createMedicalStorePartner(storeDocData, authData);
      toast.success('Medical Store Partner created successfully');
  ...
  const toggleStatus = async (store) => {
    try {
      await updateMedicalStore(store.id, { isActive: !store.isActive });
      toast.success(`Store ${store.isActive ? 'disabled' : 'enabled'} successfully`);
      fetchStores();
    } catch (error) {
  ...
  if (loading) return <LoadingSpinner message="Loading medical stores..." />;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage partner medical stores and their service areas</p>
        </div>
      ...
      <Card>
        {stores.length === 0 ? (
          <EmptyState
            icon={Plus}
            title="No medical stores yet"
            description="Add your first medical store partner to get started"
            action={<Button onClick={() => setIsModalOpen(true)}>Add Store</Button>}
          />
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="px-4 py-3 font-semibold text-gray-700">Store Name</th>
                ...
                {stores.map((store) => (
                  <tr key={store.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-gray-500">Lic: {store.drugLicenseNumber}</p>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">{store.ownerName}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 flex items-center gap-2">
                      <button 
                        ...
                        className={`p-2 rounded-lg transition-colors ${store.isActive ? 'text-orange-600 hover:bg-orange-50' : 'text-green-600 hover:bg-green-50'}`}
                        title={store.isActive ? "Disable Store" : "Enable Store"}
                      >
                        {store.isActive ? <ShieldOff size={18} /> : <ShieldCheck size={18} />}
                      </button>
                      <button 
      ...
      <Modal
        ...
        title="Add Medical Store Partner"
        size="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4 md:col-span-2">
              <h3 className="font-semibold text-gray-900 border-b pb-2">Store Information</h3>
            ...
            <div className="space-y-4 md:col-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Store Pincode *</label>
                ...
                <div className="md:col-span-2">
                  ...
                  <input type="text" name="serviceAreas" required placeholder="e.g. 302001, 302002, 302005" value={formData.serviceAreas} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 md:col-span-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" name="drugLicenseNumber" required value={formData.drugLicenseNumber} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" name="panNumber" value={formData.panNumber} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
